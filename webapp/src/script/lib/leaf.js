...
(function (Leaf) {
    var Serializer = /** @class */ (function () {
        function Serializer() {
            ...
            this.typeMap = {
                ...
                "reg": {
                    ...
                    toType: function (data) {
                        var mod = "" + (data.g && "g") + (data.m && "m") + (data.i && "i");
                        return new RegExp(data.s, mod);
                    }
                }
            };
        }
        Serializer.prototype.tokenize = function (data) {
            var _this = this;
            if (!data)
                return data;
            if (data instanceof Array) {
                return data.map(function (item) { return _this.tokenize(item); });
            }
            if (typeof data !== "object")
                return data;
            for (var type in this.typeMap) {
                var info = this.typeMap[type];
                if (data instanceof info.Cons) {
                    return _a = {},
                        _a[this.typeKey] = type,
                        _a[this.dataKey] = info.fromType(data),
                        _a;
                }
            }
            var result = {};
        ...
        Serializer.prototype.detokenize = function (v) {
            var _this = this;
            if (!v)
                return v;
            if (v instanceof Array) {
                return v.map(function (item) { return _this.detokenize(item); });
            }
            if (typeof v !== "object")
                return v;
            if (!v[this.typeKey]) {
            ...
            var info = this.typeMap[v[this.typeKey]];
            if (!info)
                return v;
        ...
        Serializer.prototype.register = function (type, config) {
            this.typeMap[type] = config;
        };
        Serializer.getDefault = function () {
...
(function (Leaf) {
    ...
    var DataTree = /** @class */ (function () {
        function DataTree(source) {
            this.source = source;
        }
        DataTree.prototype.getNode = function (path) {
            if (!path)
                return new DataNode(this);
            path = path.trim();
            var routes = path.split(".");
            var parent = this.source.data;
            var valid = false;
        ...
        DataTree.prototype.valueCanBeObject = function (v) {
            if (typeof v === "string")
                return false;
            if (typeof v === "boolean")
                return false;
        ...
        DataTree.prototype.getValueByPath = function (path) {
            if (!path)
                return this.source["_data"];
            path = path.trim();
            var routes = path.split(".");
            var parent = this.source["_data"];
            var valid = false;
            var last = routes.pop();
        ...
        DataTree.prototype.setValueByPath = function (path, v) {
            if (!path) {
                this.source.sets(v);
                return true;
            }
            ...
            routes.push(last);
            this.source.events.emit("change", (_a = {},
                _a[routes.join(".")] = old,
                _a));
            return true;
            var _a;
    ...
    var DataNode = /** @class */ (function () {
        function DataNode(tree, path) {
            ...
            Object.defineProperty(this, "v", {
                ...
                set: function (v) { return _this.set(v); }
            });
        }
        DataNode.prototype.getProxyOrValue = function () {
            var _this = this;
            var v = this.get();
            if (this.tree.valueCanBeObject(v)) {
                return new Proxy(v, {
                    get: function (obj, k) {
                        if (typeof k !== "string")
                            return;
                        var node = _this.tree.getNode(_this.path + ("" + (_this.path && ".") + k));
                        if (node)
                    ...
                    set: function (obj, k, v) {
                        if (typeof k !== "string")
                            return;
                        _this.tree.setValueByPath(_this.path + ("" + (_this.path && ".") + k), v);
                        return true;
                    }
                });
            }
        ...
        DataNode.prototype.set = function (v) {
            return this.tree.setValueByPath(this.path, v);
        };
        DataNode.prototype.get = function () {
...
(function (Leaf) {
    var ViewModel = /** @class */ (function (_super) {
        ...
        function ViewModel(_name, _fields) {
            ...
            _this.tree = new Leaf.DataTree(_this);
    ...
    var WidgetCore = /** @class */ (function (_super) {
        ...
        WidgetCore.prototype.attachText = function (text, config) {
            for (var _i = 0, _a = config.templates; _i < _a.length; _i++) {
                ...
                if (item.type !== "value")
                    continue;
                var root = Leaf.CoreLogic.getPropertyRoot(item.content);
        ...
        WidgetCore.prototype.syncDynamicProperties = function (el) {
            ...
            for (var _i = 0, _a = el.propertyConfigs || []; _i < _a.length; _i++) {
                ...
                for (var name_11 in config.rules) {
                    ...
                    var value = Leaf.CoreLogic.explainDynamicString(vm.tree, rule);
                    if (!props[name_11])
                        props[name_11] = [];
            ...
            for (var _i = 0, _a = el.propertyConfigs; _i < _a.length; _i++) {
                ...
                for (var name_14 in config.rules) {
                    if (name_14 === propertyName) {
                        values.push(Leaf.CoreLogic.explainDynamicString(config.master.ViewModel.tree, config.rules[name_14]));
                    }
                }
            }
            ...
            for (var _b = 0, todo_1 = todo; _b < todo_1.length; _b++) {
                ...
                for (var name_15 in dynamics_1) {
                    ...
                    for (var _c = 0, _d = config.templates; _c < _d.length; _c++) {
                        ...
                        if (part.type === "raw")
                            continue;
                        var root = Leaf.CoreLogic.getPropertyRoot(part.content);
                        this.widget.ViewModel.events.listenBy(el, ["change/" + root, "depChange/" + root], this.syncDynamicProperty.bind(this, el, name_15));
                    }
                }
            }
        };
        ...
        WidgetInitializer.prototype.createElement = function (tagName, properties, children) {
            ...
            for (var name_18 in properties) {
                ...
                if (typeof v === "string") {
                    ...
                    dynamics[name_18] = v;
                    el.setAttribute(name_18, Leaf.CoreLogic.explainDynamicString(this.widget.ViewModel.tree, v));
                }
            }
            el.appendChild(frag);
...
(function (Leaf) {
    var Widget = /** @class */ (function () {
        function Widget(template, name, _VMFields) {
            ...
            Leaf.Property.define(this, "VM", {
                get: function () {
                    ...
                    return _this.ViewModel.tree.getNode().getProxyOrValue();
                },
                set: function (values) {
        ...
        Widget.prototype.behave = function (handler) {
...
(function (Leaf) {
    ...
    var RPCComposer = /** @class */ (function () {
        ...
        RPCComposer.prototype.serialize = function (data) {
            return this.serializer.serialize(data);
        };
        RPCComposer.prototype.deserialize = function (content) {
        ...
        RPCComposer.prototype.handleMessage = function (rawString) {
            var data = null;
            try {
                var raw = this.deserialize(rawString);
...
(function (Leaf) {
    ...
    (function (CoreLogic) {
        ...
        CoreLogic.BubbleEvents = ["click", "mouseup", "mousedown", "mousemove", "mouseleave", "mouseenter", "mouseover", "keydown", "keyup", "keypress", "touchstart", "touchend", "touchmove", "input"].map(function (name) {
